        ...
        _touchHandler : function(e) {
            ...
            switch (e.type) {
                ...
                case 'touchend':
                    ...
                    var threshold = 10;
                    console.log("TOUCHEND",(x < (oldX + threshold) && x > (oldX - threshold) && y < (oldY + threshold) && y > (oldY - threshold)));
                    if (x < (oldX + threshold) && x > (oldX - threshold) &&
                        y < (oldY + threshold) && y > (oldY - threshold)) {
                        this._touching = false;
                        console.log("touchendthresholdpass",this.touchPrevents,this);
                        if (this.touchPrevents) {
                            var tagName = e.currentTarget.tagName;
                            console.log("touchlookcheck"+tagName);
                            if (tagName === 'BUTTON' ||
                                tagName === 'A' || tagName === 'I') {
                                e.preventDefault();
                                e.stopPropagation();
